...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        if (parent == null || (parent.leftChild != child && parent.rightChild != child))
            throw new IllegalArgumentException("Provided child is not a child of the provided parent");
            ...
            else // when parent == parent.parent.leftChild
            ...
            if (parent.parent == null)
                ...
                parent.parent.leftChild = child;
            else // when parent == parent.parent.rightChild
                parent.parent.rightChild = child;
    ...
    private void enforceRBTreePropertiesAfterInsert(Node<T> node) {
        // if there is no violation, return immediately
        if (node == root || node.parent.isBlack)
            return;
        if (node.parent.isLeftChild()) {
            Node<T> uncle = node.parent.parent.rightChild;
            // check if uncle is null or on the opposite of the child node (case 1)
            if (uncle == null || (uncle.isBlack && node.isLeftChild())) {
            ...
            // check if uncle is on the same side as the child node (case 2)
            else if (uncle.isBlack && node == node.parent.rightChild) {
            ...
            // uncle is red (case 3)
                ...
                // swap colors of parent, grandparent, uncle
                node.parent.isBlack = !node.parent.isBlack;
                node.parent.parent.isBlack = !node.parent.parent.isBlack;
                uncle.isBlack = true;

                // then, recursively check for violations between grandparent and its parent
                if (node.parent.parent != root && !node.parent.parent.isBlack && !node.parent.parent.parent.isBlack)
                    enforceRBTreePropertiesAfterInsert(node.parent.parent);
            }
        }
        // else: the parent is a right child (same code, reversed)
        else {
            Node<T> uncle = node.parent.parent.leftChild;
            // check if uncle is null or on the opposite of the child node (case 1)
            if (uncle == null || (uncle.isBlack && node == node.parent.rightChild)) {
                // color swap parent and grandparent
                node.parent.parent.isBlack = false;
                node.parent.isBlack = true;
                // rotate parent and grandparent
            ...
            // check if uncle is on the same side as the child node (case 2)
            else if (uncle.isBlack && node.isLeftChild()) {
                // rotate the two red nodes first
                rotate(node, node.parent);
                // then, do case 1:
                node.parent.isBlack = false;
